...
class Event extends Component
{
    ...
    {
        return view('livewire.panel.dashboard.event', [
    ...
    {
        $documents = $this->getEvents(false, $this->checkedDocIds());
        ...
        $time = time();
        $name = "event-{$this->user->id}-{$time}.zip";
    ...
    {
        $events = $this->getEvents()->pluck('id')->toArray();

        if ($checked) {
            foreach ($events as $event) {
                $this->check_doc[intval($event)] = true;
            }
        } else {
            foreach ($events as $event) {
                $this->check_doc[intval($event)] = false;
            }
        }
    ...
    public function getEvents($paginate = false, $ins = null)
    {
        ...
        if ($paginate) {
            return $events->paginate(env('PAGINATION_LIMIT', 5));
        }
    ...
    {
        $document = DB::table('event_documents')->where('id', $id)->first();
